...
open  class RSACrypto: NSObject {
    ...
    // pubkeyBase64: RSA public key in base64 (data between "-----BEGIN PUBLIC KEY-----" and "-----END PUBLIC KEY-----")
    static public func encryptWithRSAPublicKey2(_ data: Data, pubkeyBase64: String, keychainTag: String) -> Data? {
        let myKeychainTag = keychainTag + (Config.useKeyHashes ? "-" + String(pubkeyBase64.hashValue) : "")
        var keyRef = getRSAKeyFromKeychain(myKeychainTag)
        AppUtils.Log(from:AppUtils.self,with:"RSA keyRef [encryptWithRSAPublicKey] \(keyRef)")
        if ( keyRef == nil ) {
    ...
    //
    static public func encryptWithRSAPublicKey(dataToEncrypt: Data,publicKey:String) -> Data? {
        let seck = RSAHelper().getPublicKeyFromPEMStringConvertToSecKey(publicKey: publicKey)
        //return decryptWithRSAKey(encryptedData, rsaKeyRef: seck!, padding: SecPadding())
        return encryptWithRSAKey(dataToEncrypt, rsaKeyRef: seck!, padding: SecPadding.PKCS1)
    }
    ...
    // pubkeyBase64: RSA public key in base64 (data between "-----BEGIN PUBLIC KEY-----" and "-----END PUBLIC KEY-----")
    static public func decryptWithRSAPublicKey2( encryptedData: Data, pubkeyBase64: String, keychainTag: String) -> Data? {
        let myKeychainTag = keychainTag + (Config.useKeyHashes ? "-" + String(pubkeyBase64.hashValue) : "")
        var keyRef = getRSAKeyFromKeychain(myKeychainTag)
        if ( keyRef == nil ) {
            keyRef = addRSAPublicKey(pubkeyBase64, tagName: myKeychainTag)
    ...
    //
    static public func decryptWithRSAPublicKey(encryptedData: Data,publicKey:String) -> Data? {
        let seck = RSAHelper().getPublicKeyFromPEMStringConvertToSecKey(publicKey: publicKey)
        return decryptWithRSAKey(encryptedData, rsaKeyRef: seck!, padding: SecPadding())
    }
}

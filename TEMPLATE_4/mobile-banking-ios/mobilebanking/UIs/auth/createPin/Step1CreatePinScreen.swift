...
struct Step1CreatePinScreen: View {
 ...
 *VIEW EXTEXTIONS*
 */
extension Step1CreatePinScreen {
    ...
    func SetUpUI(geometry:GeometryProxy) -> some View {
        ZStack{
            ...
            //
            NavigationLink("", destination: Step2ConfirmCreatePinScreen(createdPin: pinCode), isActive: $navigateToConfirm).opacity(0)
            //
            VStack(spacing:10){
                ...
                //
                CustomTextBold(text: "Create new pin", textColor: .black, fontSize: 20, textAlignment: .center).vSpacingWithMaxWidth(.center)
                //

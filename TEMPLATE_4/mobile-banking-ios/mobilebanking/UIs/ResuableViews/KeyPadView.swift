    ...
    var body: some View {
        HStack(spacing: 10) {
            ForEach(0..<4) { index in
                Circle()
                    .foregroundColor(index < self.name.count ? Color(hexString: CustomColors.darkBlue) : .gray.opacity(0.5))
...
struct NumberView:View{
    ...
    var body: some View{
        Button(action: {
            ...
            Text(numberString)
                .font(.custom(CustomFontNames.NunitoSans_Bold, size: 32).weight(.bold))
